...
for i in range(len(cards)):
    ...
    for j in range(0, n_samples_per_card, batch_size):
        ...
        batch_inputs = X_test[i][j:j+batch_size]
        
        x_batch = torch.LongTensor(batch_inputs).to(device)
...
plt.plot(cards, results['deepsets']['acc'], marker='o', label='DeepSets', linewidth=2)
plt.plot(cards, results['lstm']['acc'], marker='s', label='LSTM', linewidth=2)

plt.xlabel('Card', fontsize=12)
plt.ylabel('Accuracy', fontsize=12)
plt.title('Accuracy vs #', fontsize=14)
